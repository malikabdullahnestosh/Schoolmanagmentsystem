  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const res = await fetch(`${constant.apiUrl}/gender-report`, {
          headers: {
        ...
        setGenderData(dataWithId);
      } catch (err) {
        setError("Could not load gender data.");
      } finally {
        setLoading(false);
      }
    };
    ...
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-[#2191BF] text-white p-4 rounded-t-lg flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">Gender Wise Students Report</h1>
        </div>
        ...
        <div className="bg-white rounded-b-lg shadow-lg">
          ...
          <div className="p-6 border-b border-gray-200">
            ...
            {searchTerm && (
              <div className="mt-2 text-sm text-gray-600">
                Showing {filteredData.length} of {genderData.length} results for
                "{searchTerm}"
              </div>
            )}
          </div>
          ...
          <div className="px-6 py-3 border-t border-gray-200 bg-gray-50">
            <div className="text-sm text-gray-700">
              {genderData.length > 0
                ? `Showing ${filteredData.length} of ${genderData.length} rows`
                : "No rows"}
            </div>
          </div>
        </div>
