...
const SectionsReport = () => {
  ...
  // Fetch section-wise data from API
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const res = await fetch(`${constant.apiUrl}/sections-report`, {
          headers: {
        ...
        setError("Could not load sections data.");
      } finally {
        setLoading(false);
      }
    };
  ...
  const filteredData = sectionsData.filter((item) => {
    ...
    switch (filterColumn) {
      ...
      case "classYear":
        return item.classYear?.toLowerCase().includes(searchLower);
      case "section":
      ...
      default:
        return (
          ...
          item.classYear?.toLowerCase().includes(searchLower) ||
          ...
          <div className="p-6 border-b border-gray-200">
            ...
            {searchTerm && (
              <div className="mt-2 text-sm text-gray-600">
                Showing {filteredData.length} of {sectionsData.length} results
                for "{searchTerm}"
              </div>
            )}
          </div>
          ...
          <div className="px-6 py-3 border-t border-gray-200 bg-gray-50">
            <div className="text-sm text-gray-700">
              {sectionsData.length > 0
                ? `Showing ${filteredData.length} of ${sectionsData.length} rows`
                : "No rows"}
            </div>
          </div>
        </div>
